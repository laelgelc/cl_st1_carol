#!/usr/bin/env python3
"""
Download selected Jubilee debate audio and metadata for Corpus Linguistics Study 1.

This script reads debate sample metadata from an NDJSON file, validates and
deduplicates records by "corpus_id", and uses yt-dlp to directly download
Whisper-ready audio (WAV, mono, 16kHz, s16le PCM) for each selected debate.
...
Examples:
    ...
    python download_jubilee_debates_audio.py --metadata-only
    python download_jubilee_debates_audio.py --no-test-mode
    python download_jubilee_debates_audio.py --no-test-mode --cookies env/youtube_cookies.txt
    python download_jubilee_debates_audio.py --no-test-mode --start-corpus-id jubilee_surrounded_003
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Download selected Jubilee debate audio and metadata directly with yt-dlp."
    )
...
def main() -> int:
    """Run the complete Jubilee debate audio download workflow."""
    args = parse_args()
    run_id = make_run_id()
    start_time = utc_timestamp()
